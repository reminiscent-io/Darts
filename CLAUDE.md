...
- `npm run check` — TypeScript type checking. Passes clean; any error is yours.
...
`tsconfig.json` sets `incremental: true` with its build info at `node_modules/typescript/tsbuildinfo`. That cache survives edits to `tsconfig.json` itself, so if `npm run check` reports errors that don't match the code in front of you, delete that file and run it again.
...
**Everything past the home screen is code-split** via `lazy()` in `App.tsx`, which keeps recharts out of the initial bundle. Converting one of those to a static import silently regresses first-load size. In build output the shared lazy chunk gets named after `confetti` but actually carries recharts, and at ~420 kB raw it is the largest chunk in the build. Don't read that name as meaning the confetti helper is huge.
...
- `client/public/manifest.webmanifest` — `standalone`, portrait. One icon entry: `favicon.png` at its true 512x512, `purpose: any`. There is deliberately no `maskable` entry, because the dartboard art runs edge to edge with no safe zone and Android would crop its rim. Adding one means producing a properly inset icon first, not relabelling this file.
